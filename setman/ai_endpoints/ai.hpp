...
{
  public:
    GenericClient(const std::string &api_key, CURL *curl, available_clients id)
        : api_key_(api_key), curl_(curl), id_(id)
    {
    }

    virtual ~GenericClient() noexcept
    {
        if (headers_)
            curl_slist_free_all(headers_);
        // CURL is a global resource!! clean it up urself
    }
    ...
    constexpr available_clients client_id() const { return id_; }

    virtual GenericResponse send(GenericRequest) = 0;

  protected:
    std::string api_key_;
    CURL *curl_;
    struct curl_slist *headers_ = nullptr;
    available_clients id_;
};
